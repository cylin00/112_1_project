...
with open(path, 'r') as data:
    ...
    for L in range(1, 50): #1 to 50
        ...
        for i in range(L): #0 to L-1
            ...
            for j in range(L, 2*L-1): #L to 2*L-2
                for n in range(n1, n2+1):
                    mat1[i][j] += float(original_data[n-i-1])*np.square((float(original_data[n-j+L-1])-float(original_data[n-j+L-2])))
        for i in range(L, 2*L-1): #L to 2*L-2
            for j in range(L):
                for n in range(n1, n2+1): #n1 to n2
                    mat1[i][j] += float(original_data[n-j-1])*np.square((float(original_data[n-i+L-1])-float(original_data[n-i+L-2])))
            for j in range(L, 2*L-1):
                for n in range(n1, n2+1): #n1 to n2
                    mat1[i][j] += np.square((float(original_data[n-j+L-1])-float(original_data[n-j+L-2]))) * np.square((float(original_data[n-i+L-1])-float(original_data[n-i+L-2])))
        ...
        for k in range(L, 2*L-1): #L to 2*L-2
            mat3.append(0)
            for n in range(n1, n2+1):
                mat3[k] += float(original_data[n])*np.square((float(original_data[n-k+L-1])-float(original_data[n-k+L-2])))
        ...
        for n in range(n3, n4+1): #n3 to n4
            for k in range(2*L-1):
                ...
                if k>=L: predict_data[n] += parameter[k]*np.square(float(original_data[n-k+L-1])-float(original_data[n-k+L-2]))
            predict_data[n]/=2
            print(f"prediction: {predict_data[n]}, practical: {original_data[n]}")
            mse_L += np.square(float(original_data[n]) - predict_data[n])
